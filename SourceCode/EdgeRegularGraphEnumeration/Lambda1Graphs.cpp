...
void writeTriangle(int n, int k, int l){
    //Write in neighbourhood and antineighbourhood for 1,2,3
    for(int i=2;i<=k+1;i++){
        cout << getIndex(allEdges, MP(1,i)) << " " << 0 << endl;
    }
    for(int i=k+2;i<=n;i++){
        cout << -1 * getIndex(allEdges, MP(1,i)) << " " << 0 << endl;
    }

    cout << getIndex(allEdges, MP(2,3)) << " " << 0 << endl;
    for(int i=4;i<=k+1;i++){
        cout << -1 * getIndex(allEdges, MP(2,i)) << " " << 0 << endl;
    }
    for(int i=k+2;i<=2*k-1;i++){
        cout << getIndex(allEdges, MP(2,i)) << " " << 0 << endl;
    }
    for(int i=2*k;i<=n;i++){
        cout << -1 * getIndex(allEdges, MP(2,i)) << " " << 0 << endl;
    }

    for(int i=4;i<=2*k-1;i++){
        cout << -1 * getIndex(allEdges, MP(3,i)) << " " << 0 << endl;
    }
    for(int i=2*k;i<=3*k-3;i++){
        cout << getIndex(allEdges, MP(3,i)) << " " << 0 << endl;
    }
    for(int i=3*k-2;i<=n;i++){
        cout << -1 * getIndex(allEdges, MP(3,i)) << " " << 0 << endl;
    }

    //Write in edges in the neighbourhoods
    for(int i=4;i<3*k-3;i+=2){
        cout << getIndex(allEdges, MP(i,i+1)) << " " << 0 << endl;
        if(i+2 <= 3*k-3){
            cout << -1 * getIndex(allEdges, MP(i+1,i+2)) << " " << 0 << endl;
        }
    }
}
...
void fillInRemainingGraphs(int n, int k, int l, vector<pi> edges){
    ...
    for(int i=start;i<=n;i++){
        for(int j=i+1;j<=n;j++){
            if(getIndex(edges, MP(i-start+1,j-start+1))==-1){
                cout << -1 * getIndex(allEdges, MP(i,j)) << " " << 0 << endl;
            }
            else{
                cout << getIndex(allEdges, MP(i,j)) << " " << 0 << endl;
            }
        }
    }
}
...
int main()
{
    ...
    freopen("NonIso7Graphs.txt", "r", stdin);
    ...
    for(int t=0;t<tt;t++){
        ...
        for(int eNum = 1; eNum<allEdges.size();eNum++){
            ...
            if(AEdges.size() > 0){
                ...
                //Lexicographic ordering between (if the neighbour graph has pairs 12 34 56 ..., then we enforce on 12, 34, ..., and all odd-odd)
                //Assumes k is even, then it's just if b-a==1 && a%2==0 or (b-a)%2 = 0 && a%2 = 1
                //First check if it's in the neighbour graphs
                if(4 <= a <= 3*K-3 && 4 <= b <= 3*K-3){
                    if((b-a==1 && a%2 == 0) || ((b-a)%2==0&&a%2==1)){
        ...
        //Encoding other edges
        writeTriangle(N,K,l);
        fillInRemainingGraphs(N,K,l,edges);
    }
