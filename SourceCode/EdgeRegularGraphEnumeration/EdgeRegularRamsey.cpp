...
//Encodes that neither all 10 variables are false nor true
//Used to establish R5,5 goodness
void writeRamsey(vector<int> Xs){
    for(int i=0;i<10;i++){
        cout << -1 * Xs[i] << " "; 
    }
    cout << 0 << endl;
            ...
            for(vs[2]=vs[1]+1;vs[2]<=N;vs[2]++){
                for(vs[3]=vs[2]+1;vs[3]<=N;vs[3]++){
                    for(vs[4]=vs[3]+1;vs[4]<=N;vs[4]++){
                        ...
                        for(int i=0;i<5;i++){
                            for(int j=i+1;j<5;j++){
                                Xs.PB(getIndex(allEdges,MP(vs[i],vs[j])));
                            }
                        }
                        writeRamsey(Xs);
                    }
